...
def fetch_fx_rate():
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            print(
                "Error in fetch_fx_rate: Could not retrieve exchange rate from the response."
            )
            traceback.print_exc()
            return None
    else:
        print(
            f"Error in fetch_fx_rate: API request failed with status code {response.status_code}."
        )
        traceback.print_exc()
        return None
...
def conc_calc_transfer_loss(fx_rate: float):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(fetch_and_calc, medium): medium
            for medium in list(transfer_mediums.keys())
        }
...
def conc_find_highest_premium(fx_rate: float, currencies, ex=coinone):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(calc_premium_for_symbol, symbol, ex, fx_rate): symbol
...
def is_currency_depositable(currency):
    ...
    try:
        ...
        print(f"Error in is_currency_depositable: {e}")
        traceback.print_exc()
        return False
...
def fetch_deposit_address(exchange, target, is_fetch=True):
    ...
    if is_fetch:
        try:
            ...
            print(f"Error in fetch_deposit_address with {target}: {e}")
            # traceback.print_exc()
            return None, None
    else:
        # Fetch the address and tag from the CSV file
        return get_address_from_csv("target_currency_data.csv", target)
...
def check_coinone_deposit_suspended(currency):
    ...
    try:
        ...
        print(f"Error in check_coinone_deposit_suspended: {e}")
        traceback.print_exc()
        return None
...
def fetch_coinone_currency_data(currency):
    url = f"https://api.coinone.co.kr/public/v2/currencies/{currency}"
    data = {}
    response = call_coinone_api(url, "GET", data)
    return response
...
def fetch_coinone_currency_withdraw_fee(currency):
    ...
    :param currency: The currency to fetch the withdrawal fee for (e.g., 'BTC').
    :return: The withdrawal fee for the specified currency, or None if not found.
    """
    data = fetch_coinone_currency_data(currency)
    if data:
